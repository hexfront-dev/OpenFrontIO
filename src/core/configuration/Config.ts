...
// Floors: a huge attacker's tiles cost 0.6x, a huge defender's 0.7x.
const LARGE_ATTACKER_DEPTH = 0.4;
...
export class Config {
  ...
  unitInfo(type: UnitType): UnitInfo {
    ...
    switch (type) {
      ...
      case UnitType.DefensePost:
        info = {
          cost: this.costWrapper(
            (numUnits: number) => Math.min(200_000, (numUnits + 1) * 25_000),
  ...
  samRange(level: number): number {
    ...
    return this.maxSamRange() - 990 / (level + 5);
  }
